...
if [ $sequential -eq 0 ]; then
	...
	if [[ $notification -eq 2 ]]; then
        	condorcmd="condor_submit_dag -autorescue 1 -notify never"	# we have an extended notification anyway
        	...
        	condorcmd="condor_submit_dag -autorescue 1"
                echo "Will send an e-mail notification when the benchmark completes (if mail configured)"
        else
		condorcmd="condor_submit_dag -autorescue 1 -notify never"
