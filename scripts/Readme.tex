		...
		\begin{figure}
			...
			\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=1.0cm,semithick]
				\matrix(m)[matrix of nodes,row sep=0.5cm,column sep=2cm,
					...
					|[user]| User & |[document]| \tt table.dat \\
					|[bmspecificscript]| \tt run.sh (all) & \\
					|[staticscript]| \tt runinsts.sh & |[customizeablescript]| Aggregation Script \\
					|[staticscript]| \tt runconfs.sh & |[document,copy shadow,fill=white]| \tt inst-n.out \\
					|[bmspecificscript]| \tt run.sh (single) & |[customizeablescript]| Output Builder \\
					|[staticscript]| Reasoner & |[document,copy shadow,fill=white]| \begin{tabular}{c}Reasoner Output \\ and Return Value\end{tabular} \\
				};

				{
				\tiny
        		\node[draw,fill=green!20,below=2.5cm of m-5-1,minimum width=2.5cm,anchor=west] (lbsc) {benchmark-specific};
        		\node[draw,fill=blue!20,below of=lbsc,minimum width=2.5cm] (lcs) {customizable default};
        		\node[draw,fill=black!20,below of=lcs,minimum width=2.5cm] (lss) {static};
