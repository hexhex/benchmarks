...
\begin{document}
	...
	\label{sec:usage}
		...
		the current {\tt run.sh} script is located.
		...
		\begin{figure}
			...
			\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=1.0cm,semithick]
				\matrix(m)[matrix of nodes,row sep=0.5cm,column sep=2cm,
					...
					|[staticscript]| \tt runinsts.sh & |[customizeablescript]| \tt aggregateresults.sh \\
					|[staticscript]| \tt runconfs.sh & |[document]| \tt inst-n.out \\
					|[bmspecificscript]| \tt run.sh (single) & |[customizeablescript]| \tt outputbuilder.sh \\
				};

				{
				\tiny
        		\node[draw,fill=green!20,below=1.4cm of m-5-1,minimum width=2.5cm,anchor=west] (lbsc) {benchmark-specific};
