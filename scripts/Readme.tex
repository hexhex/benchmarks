...
\begin{document}
	...
	\label{sec:usage}
		...
		\item[(3)] Here the actual command-line, without configuration-specific options, needs to be inserted.
		...
		\begin{figure}
			...
			\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=1.0cm,semithick]
				\matrix(m)[matrix of nodes,row sep=0.5cm,column sep=2cm,
					...
					|[user]| User & |[document]| table.dat \\
					|[script]| \tt run.sh (all) & \\
					|[script]| \tt runinsts.sh & |[script]| \tt aggregateresults.sh \\
					|[script]| \tt runconfs.sh & |[document]| inst.out \\
					|[script]| \tt run.sh (single) & |[script]| \tt outputbuilder.sh \\
				};
		...
		\subsection{Generating \LaTeX-Tables}
		    ...
		    \begin{itemize}
				...
				\item[{\tt \$2}:] (optional) Pattern how to format each line (without final {\tt \textbackslash\textbackslash}) \\
					The string may consists of constant parts and expressions of form
