...
\begin{document}
	...
	\label{sec:usage}
		...
		\item[(2)] Here, the configurations (of \dlvhex{} or another software system) to be compared should be entered in a semicolon-separated list.
		\item[(3)] Here, the static part of the actual shell command for a reasoner call needs to be inserted (i.e.,~without configuration-specific options).
			...
			\footnote{Comparing entirely different systems (and not only different configurations of the same system) can be realized as follows.
				The `configurations' specified at (2) should contain the \emph{full} command strings using the variable {\tt \$instance}
				for the instance name.
				The string at (3) should be {\tt "CONF"}; since {\tt CONF} in (3)
				will be substituted by the elements from the configuration string before executing the command, this will
				insert complete reasoner calls.}
		\end{enumerate}
