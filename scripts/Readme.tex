		...
		\begin{figure}
			...
			\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=1.0cm,semithick]
				\matrix(m)[matrix of nodes,row sep=0.5cm,column sep=2cm,
					...
					|[empty]| \tt run.sh (single) & |[customizeablescript]| Output Builder \\
					|[staticscript]| Reasoner ({\sc dlvhex}) & |[document,copy shadow,fill=white]| \begin{tabular}{c}Reasoner Output \\ and Return Value\end{tabular} \\
				};
				...
				\begin{pgfonlayer}{background}
					\path[draw,fill=green!20,-,line width=1.0pt] (m-2-1.south east) -- (m-2-1.north east) -- ($(m-2-1.north west) - (1cm,0)$) -- ($(m-5-1.south west) - (1cm,0)$) -- (m-5-1.south east) -- (m-5-1.north east) -- ($(m-5-1.north west) - (0.5cm,0)$) -- ($(m-2-1.south west) - (0.5cm,0)$) -- (m-2-1.south east) -- (m-2-1.north east);
				\end{pgfonlayer}
			\end{tikzpicture}
		\endpgfgraphicnamed
